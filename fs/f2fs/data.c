...
//	printk("f2fs_zone_write_end_io: updated %llu ZONE NUM: %llu origin: %llu\n",(unsigned long long)bio->bi_iter.bi_sector,(unsigned long long)SECTOR_TO_ZONE(bio->bi_iter.bi_sector),(unsigned long long)io->before_append);
...
next:
	...
	if (f2fs_sb_has_blkzoned(sbi) && btype < META && io->zone_pending_bio) {
		printk("pending? %llu",(unsigned long long)io->zone_pending_bio->bi_iter.bi_sector);
...
out:
	if (is_sbi_flag_set(sbi, SBI_IS_SHUTDOWN) ||
		...
		//	printk("out: %llu, ZONE: %llu",(unsigned long long)io->bio->bi_iter.bi_sector,SECTOR_TO_ZONE(io->bio->bi_iter.bi_sector));
		__submit_merged_bio(io);
	}
	f2fs_up_write(&io->io_rwsem);
}
